...
def plot_pca_scatter(data_scaled, labels, representative_ids, transformer_ids, n_clusters):
    """
    使用 PCA 绘制所有变压器的 2D 散点图，并突出显示选中的代表性节点
    """
    from sklearn.decomposition import PCA

    print("\n🔍 正在生成全域变压器的 2D 降维散点图 (PCA)...")
    # 使用 PCA 将高维时序数据线性降维到 2D
    ...
    for i in range(n_clusters):
        cluster_indices = np.where(labels == i)[0]
        plt.scatter(data_2d[cluster_indices, 0], data_2d[cluster_indices, 1],
                    color=palette[i], alpha=0.4, s=40, edgecolors='none',
                    label=f'Cluster {i + 1} Nodes')

    # 2. 绘制选中的代表性节点 (高亮大五角星)
    for i, rep_id in enumerate(representative_ids):
        # 找到代表性节点在数组中的索引
        rep_idx = np.where(transformer_ids == rep_id)[0][0]

        # 为了图例整洁，只在 i=0 时加一个代表性的图例标签
        star_label = 'Selected Modality Centroids' if i == 0 else None

        plt.scatter(data_2d[rep_idx, 0], data_2d[rep_idx, 1],
                    color=palette[i], edgecolor='black', marker='*', s=600,
                    linewidths=1.5, zorder=5, label=star_label)

    # 3. 设置图表格式
    plt.title('2D Projection of Modality Distribution via PCA', fontsize=16, fontweight='bold')
    ...
    # 保存图片
    save_path = 'results/pca_modality_scatter.png'
    plt.savefig(save_path, dpi=300, bbox_inches='tight')
    print(f"📊 PCA散点图已成功保存至: {save_path}")
# =================================================================
# 6. 主函数
# =================================================================
...
def main():
    ...
    try:
        ...
        plot_pca_scatter(data_scaled, labels, representatives, transformer_ids, optimal_k)
        # 8. 保存结果
        print("\n" + "=" * 50)
        ...
        with open('results/representative_transformers.txt', 'w') as f:
            for rid in representatives:
                f.write(f"{rid}\n")
            # =================================================================
            # 9. 附加分析：计算同簇普通节点的相似度 (用于支撑论文的泛化能力 Table B2)
            # =================================================================
        import random

        print("\n" + "=" * 50)
        print("📊 开始计算同一聚类簇内【普通节点】的高相似度矩阵...")

        # 1. 我们选择第一个代表性节点 (Centroid) 作为目标簇
        target_centroid = representatives[0]

        # 2. 找到这个中心点对应的簇标签 (label)
        centroid_index = list(transformer_ids).index(target_centroid)
        target_label = labels[centroid_index]

        # 3. 找出所有属于该簇，且不是中心点的“普通节点”
        normal_nodes_in_cluster = [
            transformer_ids[i] for i in range(len(transformer_ids))
            if labels[i] == target_label and transformer_ids[i] != target_centroid
        ]

        # 4. 随机挑选 5 个普通节点进行计算
        if len(normal_nodes_in_cluster) >= 5:
            # 固定随机种子保证每次跑出来的结果一样，方便写进论文
            random.seed(42)
            sampled_normal_nodes = random.sample(normal_nodes_in_cluster, 5)

            print(f"✅ 已从簇 {target_label} (中心点为 {target_centroid}) 中随机抽取了 5 个普通节点：")
            print(sampled_normal_nodes)

            # 5. 提取这 5 个普通节点的数据，计算皮尔逊相关系数 (PCC)
            # 使用 .loc 提取对应的行，并使用 .T 进行转置，使变压器 ID 变为列名
            sampled_data = pivot_df.loc[sampled_normal_nodes].T
            normal_corr_matrix = sampled_data.corr(numeric_only=True)

            print("\n📝 这 5 个普通节点的皮尔逊相关系数矩阵 (请填入论文 Table B2)：")
            print(normal_corr_matrix.round(4))
        else:
            print(f"⚠️ 该簇内的普通节点不足 5 个，当前只有 {len(normal_nodes_in_cluster)} 个，请尝试选择其他簇。")
